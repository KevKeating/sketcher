...
/**
 * @return A function that will convert an attachment point number to a name
 * @param all_names A list of all attachment point names. If this list is empty
 * (which will happen for CHEM monomers since we don't have "pretty" names for
 * their attachment points), then the names will be generated by simply
 * prefixing the number with an "R". 
 */
static std::function<std::string(int)>
get_ap_num_to_name_func(const std::vector<std::string>& all_names)
{
    if (!all_names.empty()) {
        return [&all_names](int ap_num) { return all_names[ap_num - 1]; };
    } else {
        return [](int ap_num) { return fmt::format("R{}", ap_num); };
    }
}
